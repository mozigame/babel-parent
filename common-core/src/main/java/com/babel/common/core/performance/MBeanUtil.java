...
public class MBeanUtil {  
    ...
    public static Map getMBeanInfo(){
    	Map map = new TreeMap<>();
    	map.put("runtime", getDataMap(ManagementFactory.getRuntimeMXBean()));
    	map.put("memory", getDataMap(ManagementFactory.getMemoryMXBean()));
    	map.put("system", getDataMap(ManagementFactory.getOperatingSystemMXBean()));
    	map.put("classLoading", getDataMap(ManagementFactory.getClassLoadingMXBean()));
    	map.put("compilation", getDataMap(ManagementFactory.getCompilationMXBean()));
    	map.put("thread", getDataMap(ManagementFactory.getThreadMXBean()));
    	map.put("gc", getDataMapList(ManagementFactory.getGarbageCollectorMXBeans()));
    	map.put("memoryManager", getDataMapList(ManagementFactory.getMemoryManagerMXBeans()));
//    	map.put("memoryPool", getDataMapList(ManagementFactory.getMemoryPoolMXBeans()));
    	List<MemoryPoolMXBean> mps = ManagementFactory.getMemoryPoolMXBeans();  
    	Map<String, Object> memoryPoolMap=new HashMap<>();
    	List<Map> memoryPoolList=new ArrayList<>();
        for(MemoryPoolMXBean mp : mps){
        	memoryPoolMap=new TreeMap<>();
        	memoryPoolMap.put("name", mp.getName());
        	memoryPoolMap.put("collectionUsage", mp.getCollectionUsage());
        	memoryPoolMap.put("type", mp.getType());
        	memoryPoolMap.put("usage", mp.getUsage());
        	memoryPoolMap.put("peakUsage", mp.getPeakUsage());
        	memoryPoolList.add(memoryPoolMap);
        }
        
        map.put("memoryPoolList", memoryPoolList);
        
    	return map;
    }
    
    private static Map<String, Object> getDataMap(Object obj){
    	Map<String, Object> map=new TreeMap<>();
    	try {
    		map= ObjectToMapUtil.objectToMap(obj);
		} catch (Exception e) {
    ...
    private static List<Map<String, Object>> getDataMapList(List list){
    	List<Map<String, Object>> mapList=new ArrayList<>();
    
    	for(Object obj:list){
	    	Map<String, Object> map=new TreeMap<>();
	    	try {
	    		if(obj instanceof GarbageCollectorMXBean){
	    			String ignoreProps="lastGcInfo";
	    			map=ObjectToMapUtil.getDataMapByPropName(obj, ignoreProps, "ignore");
	    		}
	    		else{
	    			map= ObjectToMapUtil.objectToMap(obj);
	    		}
