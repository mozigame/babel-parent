...
public abstract class BaseEntityIdSimple implements Serializable {
...
//		this.status=1;
	...
	public static Set<Long> getOperaterIds(Collection<Object> list) {
		...
		if(obj instanceof BaseEntityIdSimple){//检查第一个对象是否继承BaseEntity
			BaseEntityIdSimple b=null;
			for(Object o:list){
				b=(BaseEntityIdSimple)o;
	 ...
	 * @return
	 */
	public static List<Long> getIdList(Collection list) {
	    if(!CollectionUtils.isEmpty(list)) {
	        Object object=list.iterator().next();
	        if(object instanceof BaseEntityIdSimple) {
	            Collection<BaseEntityIdSimple> eList=list;
	            List<Long> idList=new ArrayList<>();
	            for(BaseEntityIdSimple o:eList){
	                idList.add(o.getId());
	            }
	            return idList;
	        }
	        else if(object instanceof BaseEntitySimple) {
	        ...
	        else if(object instanceof BaseEntityTimestamp) {
	            ...
	            Collection<BaseEntity> eList=list;
	            List<Long> idList=new ArrayList<>();
	            for(BaseEntity o:eList){
	                idList.add(o.getCid());
	            }
	            return idList;
	        }
